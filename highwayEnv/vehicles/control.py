...
class AgentVehicle(Vehicle):
    ...
    def mutate_threshold(self, sigma):
        """Change the agent risk threshold by mutation."""
        if  random.random() <= variables.mutation_probability:
    ...
    def visualize (self, non_dominated_time, dominated_time, non_dominated_risk, dominated_risk):
        """Create a plot of the different options for the agent based on risk and time."""
        fig, ax = plt.subplots()
        fig.subplots_adjust(bottom=0.4)
        hyperv = archive.hypervolume(non_dominated_risk, non_dominated_time)
    ...
    def first_arrival(self, roundabout):
        ...
        self.hypervolume = archive.hypervolume(non_dominated_risk, non_dominated_time)
        archive.search_archive(self, roundabout.risk_tol, roundabout.threshold_tol, roundabout.hv_tol)
    ...
    def first_arrival_test(self, roundabout):
        ...
        self.non_dominated_time = non_dominated_time
        self.hypervolume = archive.hypervolume(non_dominated_risk, non_dominated_time)
        archive.search_test_archive(self, roundabout.risk_tol, roundabout.hv_tol, roundabout.distances_to_solutions)
        self.select_solution(non_dominated_time, non_dominated_risk)
    ...
    def still_standing(self, roundabout):
        ...
        if self.timer >= self.waiting_time:
            if self.solution_selected != -1:
                ...
                variables.lower_risk_bound = self.risk_threshold + 0.01 # We do not need to explore solutions lower than this in the future
                self.mutate_threshold(roundabout.sigma)
                self.stopped = False

                
        if roundabout.steps % 2 == 0 and self.solution_selected == -1:
            risk, time = self.waiting_time_and_risk(roundabout.other_vehicles)
            non_dominated_time, dominated_time, non_dominated_risk, dominated_risk = self.get_PO_solutions(time,risk)
            self.select_solution(non_dominated_time, non_dominated_risk)
    ...
    def roundabout_entrance(self, roundabout):
        ...
        elif self.position[1] < -168 or self.position[1] > -44:
