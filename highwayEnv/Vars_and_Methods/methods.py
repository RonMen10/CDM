...
#where is the absolute value taken from ?

def not_zero(x):
    if abs(x) > EPSILON:   #but abs(x) will any ways be > 0???!
        return x
    elif x > 0:
        return EPSILON
    else:
...
# (x mod 2 -1)*np.pi but why normalize as np.pi
...
#returning  RxMin<=Xindividual<=RxMax and RyMin <= Yindividual<=RyMax
#to check if the point is inside the rectange and give out the indices of the position (x,y) belonging to that specific individual
#this function is for all the rectangles in general from the simulation
...
def point_in_rotated_rectangle(point, center, length, width, angle):
    ...
    """
    c, s = np.cos(angle), np.sin(angle)  #say angle =120
    r = np.array([[c, -s], [s, c]])  # [[ 0.81418097 -0.58061118] [ 0.58061118  0.81418097]]
    ru = r.dot(point - center) #returns dot product of two np arrays
...
def has_corner_inside(rect1, rect2): #this is for the road connecting both the roundabouts, just the straight roand not the curved rectangels
    """
        Check if rect1 has a corner inside rect2 (overlaps)
    :param rect1: (center, length, width, angle)
    :param rect2: (center, length, width, angle)
    """
    (c1, l1, w1, a1) = rect1 #not sure what the angle is for
    (c2, l2, w2, a2) = rect2
    c1 = np.array(c1)
    l1v = np.array([l1/2, 0])
    w1v = np.array([0, w1/2])
    r1_points = np.array([[0, 0],
                          - l1v, l1v, w1v, w1v,
                          - l1v - w1v, - l1v + w1v, + l1v - w1v, + l1v + w1v]) #not sure why this combination like the axis
    ...
    #return point_in_rotated_rectangle(center1+non single dimension entries from the shape of array in rotated_r1_points, c2,...)
    #                                 (point, c2,l2,w2,a2)
...
def remap(v, x, y):
    return y[0] + (v-x[0])*(y[1]-y[0])/(x[1]-x[0]) # v is a scalar , (y2-y1)/(x2-x1)


def class_from_path(path):
    module_name, class_name = path.rsplit(".", 1)
    class_object = getattr(importlib.import_module(module_name), class_name) #Python getattr() function is used to get the value of an object's attribute and if no attribute of that object is found, default value is returned. Basically, returning the default value is the main reason why you may need to use Python getattr() function.
    return class_object
