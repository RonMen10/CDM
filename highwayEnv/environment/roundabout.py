...
class RoundaboutEnv(gym.Env):
    ...
    def _is_terminal(self):
        ...
        if (self.total_attempts >= variables.training_terminal):
            pickle.dump(archive.get_archive(), open(variables.path + '/Archives/{0}_{1}_{2}_{3}.pkl'.format(self.risk_tol, self.threshold_tol, self.hv_tol, self.sigma), 'wb'))
            print(archive.get_archive())
    ...
    def step(self, action):
        ...
        for agent in self.ego_vehicles:
            ...
            if agent.lane_index in [("sxn","ner", 0), ("nx","nxs", 0)] and agent.archive_updated == False:
                archive.update_archive(agent, self.risk_tol, self.threshold_tol, self.hv_tol, agent.archive_index,
                                        agent.hypervolume, agent.non_dominated_risk)
                agent.mutate_threshold(self.sigma)
                agent.solution_selected = -1
        ...
        # Save the archive every 500 steps.
        if (self.steps % 500 == 0):
            ...
            archive_df=pd.DataFrame({ key: archive.get_archive()[key] for key in keys })
            archive_df = archive_df.sort_values(by=['hypervolume','threshold'])
            self.number_of_entries.append((self.steps, len(archive_df.index)))
            self.number_of_crashes.append((self.steps, sum(archive.get_archive()['cumCrashes'])))
