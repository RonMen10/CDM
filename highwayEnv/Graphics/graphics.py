...
class EnvViewer(object):
    ...
    def __init__(self, env, offscreen=False):
        ...
        pygame.init() # initializes all the modules of pygame for use
        pygame.display.set_caption("Highway-env") # to Get the current window caption on the simulator
        panel_size = (self.env.config["screen_width"], self.env.config["screen_height"]) # this is with regards to the simulator screen width height adjustmemnt
        ...
        if not self.offscreen:     # if offscreen is not set to true cofigure the simulator
            self.screen = pygame.display.set_mode([self.env.config["screen_width"], self.env.config["screen_height"]])#Initialize a window or screen for display
